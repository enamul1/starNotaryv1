...
it('can change owners', async () => {
 ...
 it('change name', async () => {
    let newName = "Enamul";
    let instance = await StarNotary.deployed(); // Making sure the Smart Contract is deployed and getting the instance.
    await instance.changeName(newName); // Calling the Smart Contract function claimStar
    let name = await instance.starName.call(); // Getting the owner address
    assert.equal(name, newName); // Verifying if the owner address match with owner of the address
});